...
def load_model(loaded_model,hidden_size):
...
def predict(args, model, cat_to_name):
    ...
    with torch.no_grad():
        if args.gpu and predict_on_gpu:
            ...
            output = model(image)
        top_probs, top_labels =  output.topk(args.topk)
    ...
    return top_probs.numpy()[0], mapped_classes
...
def main():
    ...
    model = load_model(args.loaded_model,args.hidden_size)

    top_probs,top_class = predict(args,model,cat_to_name)
